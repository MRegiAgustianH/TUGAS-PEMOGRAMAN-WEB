...
<body>
    ...
    <div id="transaksi-pubg">
        <form action="transaksi-pubg.php" method="post">
            ...
            <input style=" border: 1px solid black" type="text" name="jumlah_uc" id="jumlah_uc" class="input" value="<?php echo htmlspecialchars($_GET['jumlah_uc'] ?? ''); ?>" required>
...
if (isset($_POST['submit'])) {
    ...
    // Prepare and bind
    $stmt = $conn->prepare("INSERT INTO transaksi_pubg (nama, jumlah_uc, harga, metode_pembayaran, tgl_pembelian, status) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssss", $nama, $jumlah_uc, $harga, $metode_pembayaran, $tgl_pembelian, $status);

    // Execute the statement
    if ($stmt->execute()) {
        echo "<script>alert('Transaksi Berhasil');</script>";
        echo "<script>window.location.href = 'proses-tr-pubg.php?nama={$nama}&jumlah_uc={$jumlah_uc}&harga={$harga}&metode_pembayaran={$metode_pembayaran}&tgl_pembelian={$tgl_pembelian}&status=Behasil';</script>";
    } else {
