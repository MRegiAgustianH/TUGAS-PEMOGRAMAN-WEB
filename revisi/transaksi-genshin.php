...
<body>
    ...
    <div id="transaksi-genshin">
        <form action="transaksi-genshin.php" method="post">
            <label for="nama">Nama Pembeli:</label>
            <input style=" border: 1px solid black" type="text" name="nama" id="nama" class="input" required>

            <label for="jumlah_primo">Jumlah Primo:</label>
            <input style=" border: 1px solid black" type="text" name="jumlah_primo" id="jumlah_primo" class="input" value="<?php echo htmlspecialchars($_GET['jumlah_primo'] ?? ''); ?>" required>
...
if (isset($_POST['submit'])) {
    ...
    // Prepare and bind
    $stmt = $conn->prepare("INSERT INTO transaksi_genshin (nama, jumlah_primo, harga, metode_pembayaran, tgl_pembelian, status) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssss", $nama, $jumlah_primo, $harga, $metode_pembayaran, $tgl_pembelian, $status);

    // Execute the statement
    if ($stmt->execute()) {
        echo "<script>alert('Transaksi Berhasil');</script>";
        echo "<script>window.location.href = 'proses-tr-genshin.php?nama={$nama}&jumlah_primo={$jumlah_primo}&harga={$harga}&metode_pembayaran={$metode_pembayaran}&tgl_pembelian={$tgl_pembelian}&status=Behasil';</script>";
    } else {
