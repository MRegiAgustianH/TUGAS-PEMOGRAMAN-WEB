...
<body>
    ...
    <div id="transaksi-valorant">
        <form action="transaksi-valorant.php" method="post">
            ...
            <input style=" border: 1px solid black" type="text" name="jumlah_vp" id="jumlah_vp" class="input" value="<?php echo htmlspecialchars($_GET['jumlah_vp'] ?? ''); ?>" required>
...
if (isset($_POST['submit'])) {
    ...
    // Prepare and bind
    $stmt = $conn->prepare("INSERT INTO transaksi_valorant (nama, jumlah_vp, harga, metode_pembayaran, tgl_pembelian, status) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssss", $nama, $jumlah_vp, $harga, $metode_pembayaran, $tgl_pembelian, $status);

    // Execute the statement
    if ($stmt->execute()) {
        echo "<script>alert('Transaksi Berhasil');</script>";
        echo "<script>window.location.href = 'proses-tr-valorant.php?nama={$nama}&jumlah_vp={$jumlah_vp}&harga={$harga}&metode_pembayaran={$metode_pembayaran}&tgl_pembelian={$tgl_pembelian}&status=Behasil';</script>";
    } else {
